Feature: 캠핑장 예약 생성
  ...
  Scenario: 30일 제한 규칙을 확실하게 넘는 위반으로 예약 실패
    Given 오늘 날짜가 "2025-01-01"이다
    When 시작일을 31일 이후로 예약을 신청한다
    Then 예약이 실패한다
    And "오늘로부터 30일 이내로 예약할 수 있습니다" 오류 메시지가 반환된다

  Scenario: 30일 제한 규칙을 애매하게 넘는 위반으로 예약 실패
    Given 오늘 날짜가 "2025-01-01"이다
    When 시작일을 30일 이내로, 종료일을 31일 이후로 예약을 신청한다
    Then 예약이 실패한다
    And "오늘로부터 30일 이내로 예약할 수 있습니다" 오류 메시지가 반환된다

  Scenario: 종료일이 시작일보다 이전인 경우 예약 실패
    When 종료일이 시작일보다 이전으로 예약을 시도한다
    Then 예약이 실패한다
    And "종료일이 시작일보다 이전일 수 없습니다" 오류 메시지가 반환된다

  Scenario: 예약 일자가 확실하게 겹치면 예약 실패
    Given "A-1" 캠핑장에 "2025-01-15"부터 "2025-01-17"까지 예약이 존재한다
    When 다른 고객이 "A-1" 캠핑장을 이틀 이상이 겹치도록 예약을 신청한다
    Then 예약이 실패한다
  ...
  Scenario: 예약 일자가 애매하게 겹치면 예약 실패
    ...
    When 다른 고객이 "A-1" 캠핑장을 하루만 겹치도록 예약을 신청한다
    Then 예약이 실패한다
    And "해당 기간에 이미 예약이 존재합니다" 오류 메시지가 반환된다

  Scenario: 동시 예약 요청 시 하나만 성공
    When 여러 고객이 동시에 같은 캠핑장에 예약을 요청한다
    Then 하나의 예약만 성공한다
...
Feature: 캠핑장 예약 취소
  ...
  Scenario: 사전 취소 성공
    When 예약 "1"을 확인 코드 "ABC123"으로 취소 요청한다
    Then 예약 취소가 성공한다
  ...
  Scenario: 당일 취소
    ...
    And "ABC123"로 확인 코드가 발급된 예약 1을
    When 캠핑 이용 시작일이 오늘일 때 취소 요청한다
    Then 예약 취소가 성공한다
    And 예약 상태가 "CANCELED_SAME_DAY"로 변경된다

  Scenario: 잘못된 확인 코드로 취소 실패
    Given "ABC123"로 코드로 발급된 예약 "1"이 존재한다.
    When 확인 코드 "WRONG123"으로 취소 요청한다
    Then 예약 취소가 실패한다
    And "확인 코드가 일치하지 않습니다" 오류 메시지가 반환된다
